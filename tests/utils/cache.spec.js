...
describe("测试cache.js", () => {
  it("不是用缓存的方式", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test0", null);
  ...
  it("不是用缓存的方式{ cache: false }", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test1", null, { cache: false });
  ...
  it("使用缓存，设置参数为{ cache: true }", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test1", null, { cache: true });
  ...
  it("使用缓存，设置参数为{ cache: true, expire: 10 }", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test2", null, { cache: true, expire: 10 });
  ...
  it("使用缓存，设置参数为{ cache: true, expire: 10 }", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test3", null, { cache: true });
    const getTimeSpanWithCache1 = cacheDec(getTimeSpan, "test3", null, { cache: true });
    const getTimeSpanWithCache2 = cacheDec(getTimeSpan, "test3", null, { cache: true, reload: true });
  ...
  it("使用缓存，方法执行返回错误，设置参数为{ cache: true }", async function() {
    this.timeout(5000);
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test4", null, { cache: true });
  ...
  it("使用缓存，使用第三个参数，{ cache: true }", async function() {
    ...
    const getTimeSpanWithCache = cacheDec(getTimeSpan, "test5", localStorage, { cache: true, local: true });
